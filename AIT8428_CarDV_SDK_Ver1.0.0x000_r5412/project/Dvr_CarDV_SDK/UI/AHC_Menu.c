...
#define CFG_POWER_ON_ALWAYS_REC
/*===========================================================================
 * Global variable
 *===========================================================================*/ 
...
{
     //printc("StateModeDoHandlerEvent--ulMsgId:x%X\r\n", ulMsgId);
    switch(ulMsgId){
#if (defined(WIFI_PORT) && WIFI_PORT == 1)
        ...
        case AHLM_GPIO_BUTTON_NOTIFICATION:
    		...
    		//printc("StateModeDoHandlerEvent--ulEvent:x%X\r\n", ulEvent);
               ...
               // printc("StateModeDoHandlerEvent--ulFuncMain:x%X--ulFuncSub:x%X--ulFuncMisc:x%X \r\n", ulFuncMain,ulFuncSub,ulFuncMisc);
...
void VideoSettingInit( void )
{
    ...
    //Set ADAS parameter
    {
	    ...
	    printc("VideoSettingInit---bLDWS_En=%d\n",bLDWS_En);
		...
		printc("VideoSettingInit---bFCWS_En=%d\n",bFCWS_En);
        ...
        if ( AHC_Menu_SettingGetCB( (char *)COMMON_KEY_SAG_EN, &bSAG_En ) == AHC_TRUE ) {
	      printc("VideoSettingInit---bSAG_En=%d\n",bSAG_En);
...
AHC_BOOL uiStateProcessGlobalEvent2(UINT32 ulMsgId, UINT32* pulEvent, UINT32* pulParam)
{
    ...
    switch(*pulEvent)
    {
		...
		AHC_WMSG_DrawBMP (WMSG_PLUG_OUT_SD_CARD, 2);
        ...
        case BUTTON_CLEAR_WMSG:
            //AHC_WMSG_Draw(AHC_FALSE, WMSG_NONE, 0);//leo20180704
		AHC_WMSG_DrawBMP(WMSG_CLEAR_SECTION,0);
            #if UVC_HOST_VIDEO_ENABLE
            if (uiGetCurrentState() == UI_BROWSER_STATE) {
