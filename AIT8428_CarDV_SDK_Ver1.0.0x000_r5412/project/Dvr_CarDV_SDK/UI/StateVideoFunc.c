...
extern MMP_ULONG OldDeskClock;
extern AHC_BOOL Icon_Clear_Flag;
extern AHC_BOOL  boot_init_flag;
static int         exitparking_flog=0;
...
UINT32				Motion_Parking_Rec	= AHC_FALSE;//leo20180630
...
void VideoRecMode_PreviewUpdate(void)
{
    ...
    VideoTimer_Start(VIDEO_TIMER_UNIT);
   AHC_SendAHLMessage(AHLM_UI_NOTIFICATION, BUTTON_REC_REL, 0);//leo20180703
...
void StateVideoRecMode_StartRecordingProc(UINT32 ulJobEvent)
{
    ...
    if(uiGetParkingModeEnable() == AHC_TRUE)
    {
        AHC_UF_SelectDB(DCF_DB_TYPE_2ND_DB);
			 Selectfile=1;//leo20180705

    }
    else
    {
        AHC_UF_SelectDB(DCF_DB_TYPE_1ST_DB);
			 Selectfile=0;//leo20180705
...
DECL_AHC_EV_HANDLER(STATE_VIDEO_REC_MODE_EVENT_KEY_MENU_LCD)
{
	...
	g_bDrawUnfix = MMP_FALSE;
printc("STATE_VIDEO_REC_MODE_EVENT_KEY_MENU_LCD\n");
    if (!VideoFunc_RecordStatus())
    {
    	printc("STATE_VIDEO_REC_MODE_EVENT_KEY_MENU_LCD1111111111\n");
...
{
    printc(FG_BLUE("@@@STATE_VIDEO_REC_MODE_EVENT_LOCK_FILE_G_LCD@@@ EVENT_LOCK_FILE_G\r\n"));
         ...
         (VideoFunc_RecordStatus() && !VideoFunc_LockFileEnabled()) )
    {
        if (m_ubGsnrIsObjMove == AHC_TRUE)
        {
            ...
            {
                //AHC_WMSG_Draw(AHC_TRUE, WMSG_LOCK_CURRENT_FILE, 2);
                 //AHC_WMSG_DrawBMP( WMSG_LOCK_CURRENT_FILE, 2);//leo20180704
				AHC_PlaySoundEffect(AHC_SOUNDEFFECT_FILELOCK);//leo20180704
                EnableLockFile(AHC_TRUE, VR_LOCK_FILE_TYPE);
                m_ulLockEventNum++;
            }
        #else
...
{
    printc("STATE_VIDEO_REC_MODE_EVENT_VR_EMERGENT_LCD\n");
    if( uiGetParkingModeEnable() == AHC_TRUE ) {
    	return;
    }
...
DECL_AHC_EV_HANDLER(STATE_VIDEO_REC_MODE_EVENT_VR_EMERGENT_LCD)
{
    ...
    if( (AHC_VIDEO_IsEmergRecStarted() == AHC_FALSE) && (AHC_VIDEO_IsEmergPostDone() == AHC_TRUE))
    {
        if (VideoFunc_RecordStatus())
        {
            printc(FG_PURPLE("STATE_VIDEO_REC_MODE_EVENT_VR_EMERGENT_LCD--EVENT_VR_EMERGENT: Go !!!! \r\n")); 
            AHC_VIDEO_SetEmergRecStarted(AHC_TRUE);
            AHC_VIDEO_SetEmergPostDone(AHC_FALSE);
            
            //AHC_WMSG_Draw(AHC_TRUE, WMSG_LOCK_CURRENT_FILE, 2);
            AHC_WMSG_DrawBMP( WMSG_LOCK_CURRENT_FILE, 2);//leo20180704
	  // if(MenuSettingConfig()->uiGPSStamp==0)
	  {
		AHC_PlaySoundEffect(AHC_SOUNDEFFECT_FILELOCK);//leo20180704
	   }
            EnableLockFile(AHC_TRUE, VR_LOCK_FILE_TYPE);
...
DECL_AHC_EV_HANDLER(STATE_VIDEO_REC_MODE_EVENT_PARKING_KEY_LCD)
{
	...
	printc("STATE_VIDEO_REC_MODE_EVENT_PARKING_KEY_LCD\n");
	if( MenuSettingConfig()->uiMotionDtcSensitivity == MOTION_DTC_SENSITIVITY_OFF ){
		return;
	}	
    ...
    //Enable Parking mode.
    printc("STATE_VIDEO_REC_MODE_EVENT_PARKING_KEY_LCD--uiGetParkingModeEnable()=%d\n",uiGetParkingModeEnable());

   AHC_SetParam( PARAM_ID_ADAS, 0 );
  if( uiGetParkingModeEnable() == AHC_FALSE ){
	...
	AHC_SendAHLMessage(AHLM_UI_NOTIFICATION, BUTTON_REC_REL, 0);//leo20180705
...
#endif
    	//printc("StateVideoRecModeHandler--ulVideoEvent:x%X\r\n", ulVideoEvent);
...
AHC_BOOL StateSelectFuncVideoRecordMode(void)
{
...
#endif
    {
        ...
        RegisterEventCb(EVENT_VIDEO_TOUCH_MENU,					STATE_VIDEO_REC_MODE_EVENT_KEY_MENU_LCD);//leo20180630
...
#else
        RegisterEventCb(EVENT_LOCK_FILE_G, 		        STATE_VIDEO_REC_MODE_EVENT_LOCK_FILE_G_LCD);//leo20180706
