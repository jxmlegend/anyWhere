...
void DrawStateMovPbStateWithTouchButton(void)
{
    ...
    //UIDrawMovPBFuncEx(bID0, MOVPB_GUI_STATUS, AHC_TRUE, AHC_TRUE, OSD_COLOR_TRANSPARENT, NULL_PARAM);//20180703

    BEGIN_LAYER(bID0);

    AHC_OSDSetColor(bID0, OSD_COLOR_TRANSPARENT);
    //AHC_OSDDrawFillRect(bID0, 90, 50, AHC_GET_ATTR_OSD_W(bID0), 190);//leo20180703
...
void DrawStateMovPbInit(void)
{
    ...
    for (idx = 0; idx < MOVPB_GUI_MAX; idx++)
    {
	 ...
	 OSDDraw_MainTouchplayback(bID0, UI_PLAYBACK_STATE);//leo20180703
    //OSDDraw_MainTouchButton(bID0, UI_PLAYBACK_STATE);
...
{
    UINT8    bID0 = 0,bID1 = 1;
	static  AHC_BOOL  pb_menu_clear_flag = AHC_FALSE;
	static UINT8 	SD_status = AHC_FALSE;
#if (QUICK_FILE_EDIT_EN)
    static UINT8 	Lock_status = AHC_FALSE;
#endif
	  RECT  DELBOTTON  = {80, 215, 50, 55};//leo20180703
    ...
    AHC_OSDRefresh_PLCD();
    #endif
	if(pb_menu_clear_flag==AHC_FALSE)//leo20180703
		pb_menu_clear_count++;
	if(pb_menu_clear_count>20&&pb_menu_clear_flag==AHC_FALSE)
	{
		//OSDDraw_EnterDrawing(&bID0, &bID1);
		BEGIN_LAYER(bID0);
		//AHC_OSDSetColor(bID0, OSD_COLOR_TRANSPARENT);
	   	//AHC_OSDDrawFillRect(bID0, 80, 215, 320, 55);
	    AHC_OSDDrawBitmap(bID0, &bmdel_button_null, 						DELBOTTON.uiLeft, 	 DELBOTTON.uiTop);
	    AHC_OSDDrawBitmap(bID0, &bmdel_button_null, 						BACK1BOTTON.uiLeft, 	 BACK1BOTTON.uiTop);
	    AHC_OSDDrawBitmap(bID0, &bmdel_button_null, 						PLAYBOTTON.uiLeft, 	 PLAYBOTTON.uiTop);
	    AHC_OSDDrawBitmap(bID0, &bmdel_button_null, 						NEXT1BOTTON.uiLeft, 	 NEXT1BOTTON.uiTop);
	    AHC_OSDDrawBitmap(bID0, &bmdel_button_null, 						HOMEBOTTON.uiLeft, 	 HOMEBOTTON.uiTop);

		END_LAYER(bID0);	
		//OSDDraw_ExitDrawing(&bID0, &bID1);
		pb_menu_clear_flag=AHC_TRUE;
		//("pb_menu_clear_flag=%d\n",pb_menu_clear_flag);
	}

	if(pb_menu_clear_count==0&&pb_menu_clear_flag==AHC_TRUE)
	{
		//OSDDraw_EnterDrawing(&bID0, &bID1);
		BEGIN_LAYER(bID0);
		OSDDraw_MainTouchplayback(bID0, UI_PLAYBACK_STATE);
		END_LAYER(bID0);	
		//OSDDraw_ExitDrawing(&bID0, &bID1);
		pb_menu_clear_flag=AHC_FALSE;
	}
}
