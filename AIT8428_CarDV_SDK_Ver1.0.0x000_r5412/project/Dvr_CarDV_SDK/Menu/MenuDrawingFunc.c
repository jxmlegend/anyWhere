...
static UINT16   usShiftStringRight = 20;
 ...
 AHC_BOOL attach_flag1 = 1 ;
AHC_BOOL attach_flag2 = 1 ;
/*===========================================================================
 * Extern function
 *===========================================================================*/
...
void MenuMainDrawBackCurtain(UINT16 uwDispID, GUI_COLOR bkColor)
{
   ...
   // AHC_OSDDrawFillRect(uwDispID, 0, 36,  DispBufWidth, 40);
    ...
    AHC_OSDSetColor(uwDispID, OSD_TOUCH_BACKGROUND);
...
void MenuDrawItemEx(UINT16 uwDispID, PSMENUSTRUCT pMenu)
{
    ...
    {
        GUI_COLOR  colorBk  = OSD_COLOR_GREEN;
        
	switch(i)
	{	
		case 0:
			barID   = bmIcon_play_normal;
			//printc("MenuDrawItemEx i=0\n");
			break;
			
		case 1:
			barID   = bmIcon_system_normal;
			//printc("MenuDrawItemEx i=1\n");
			break;

		case 2:
			barID   = bmIcon_setup_normal;
			//printc("MenuDrawItemEx i=2\n");			
			break;

		case 3:
			//printc("MenuSettingConfig()->uiMOVSize=%d\n",MenuSettingConfig()->uiMOVSize);
			//if(MenuSettingConfig()->uiMOVSize==0)flag_p
			if(flag_p)
				barID   = bmIcon_hd_button_normal;
			else
				barID   = bmIcon_hd_button_push;// lyj 20180711
			//printc("MenuDrawItemEx i=3\n");
			break;

		default:
			barID   = bmIcon_play_normal;
			//printc("MenuDrawItemEx default\n");
			break;
		...
		{	
			case 0:
				barID   = bmIcon_play_push;
				//printc("MenuDrawItemEx i=0\n");
				break;
				
			case 1:
				barID   = bmIcon_system_push;
				//printc("MenuDrawItemEx i=1\n");
				break;

			case 2:
				barID   = bmIcon_setup_push;
				//printc("MenuDrawItemEx i=2\n");			
				break;
			...
			case 3:
				#if 0
				printc("MenuSettingConfig()->uiMOVSize=%d\n",MenuSettingConfig()->uiMOVSize);
				if(MenuSettingConfig()->uiMOVSize==0)
					barID   = bmIcon_hd_button_normal;
				else
					barID   = bmIcon_hd_button_normal;
				#endif
...
void MenuDrawTitle(UINT16 uwDispID, UINT32 uiStrID)
{
    ...
    OSD_Draw_Icon(bmIcon_cam_normal, RECTExit, uwDispID);
    #else
...
void MenuDrawMainPage(PSMENUSTRUCT pMenu)
{
    ...
    AHC_OSDGetBufferAttr(bID0, &dummy, &dummy, &color, &addr);
        ...
        OSDDraw_EnableSemiTransparent(bID0, AHC_FALSE);
...
void MenuDrawChangeItem2(AHC_BOOL changeP)
{
	 UINT8           bID0    = 0;
      UINT8           bID1    = 0;
	  RECT    tmpRECT9     = {345,175,110,110};

	   OSDDraw_EnterMenuDrawing(&bID0, &bID1);

		if(changeP)
		{
			OSD_Draw_Icon(bmIcon_hd_button_normal, tmpRECT9, bID0);
			printc("~~~~~~~~~11111~~~~~~~~~~~~~\r\n");
		}
		else
		{

			OSD_Draw_Icon(bmIcon_hd_button_push, tmpRECT9, bID0);
			printc("~~~~~~~~~2222~~~~~~~~~~~~~\r\n");
		}
...
void MenuDrawChangeItem(PSMENUSTRUCT pMenu, UINT32 uiCurrItem, UINT32 uiPrevItem, UINT32 uiPreSelected)
{
    ...
    else
    {
       ...
       // MenuDraw_GetPageInfo(&iCurPage, &iTotalPage, NULL, PageItemNum);
        ...
        #else   // FONT_LARGE
	//printc("MenuDrawChangeItem-----vItem=%d\n",uiPrevItem);
	 switch(uiPrevItem)
	 {
	 	case 0:
	        MenuDrawItem(bID0, pPrevItem, uiPrevItem%PageItemNum, pPrevItem->uiStringId,
	                     bmIcon_play_normal, TextClr, clrFill, pSubMenu->bCustom);
			break;
		case 1:
	        MenuDrawItem(bID0, pPrevItem, uiPrevItem%PageItemNum, pPrevItem->uiStringId,
	                     bmIcon_system_normal, TextClr, clrFill, pSubMenu->bCustom);
			break;
		case 2:
	        MenuDrawItem(bID0, pPrevItem, uiPrevItem%PageItemNum, pPrevItem->uiStringId,
	                     bmIcon_setup_normal, TextClr, clrFill, pSubMenu->bCustom);
			break;
		case 3:
			if(MenuSettingConfig()->uiMOVSize==0)
			{
				MenuDrawItem(bID0, pPrevItem, uiPrevItem%PageItemNum, pPrevItem->uiStringId,
	                     bmIcon_hd_button_normal, TextClr, clrFill, pSubMenu->bCustom);
			}
			else
			{
			        MenuDrawItem(bID0, pPrevItem, uiPrevItem%PageItemNum, pPrevItem->uiStringId,
	...
	switch(uiCurrItem)
	 { 
	 	case 0:
        		MenuDrawItem(bID0, pCurItem, uiCurrItem%PageItemNum, pCurItem->uiStringId,
                     bmIcon_play_push, TextClr, clrFill, pSubMenu->bCustom);
			break;
		case 1:
			  MenuDrawItem(bID0, pCurItem, uiCurrItem%PageItemNum, pCurItem->uiStringId,
                     bmIcon_system_push, TextClr, clrFill, pSubMenu->bCustom);
 
			break;
		case 2:
			  MenuDrawItem(bID0, pCurItem, uiCurrItem%PageItemNum, pCurItem->uiStringId,
                     bmIcon_setup_push, TextClr, clrFill, pSubMenu->bCustom);
		...
		case 3:
			...
			{
				MenuDrawItem(bID0, pCurItem, uiCurrItem%PageItemNum, pCurItem->uiStringId,
                     bmIcon_hd_button_push, TextClr, clrFill, pSubMenu->bCustom);
			}
			else
			{
			        MenuDrawItem(bID0, pCurItem, uiCurrItem%PageItemNum, pCurItem->uiStringId,
...
AHC_BOOL display_flag1 = 0;
AHC_BOOL display_flag2 = 0;
AHC_BOOL display_flag3 = 0;
void DrawSubItem(UINT16 uwDispID, int iItem, int iTotalItems, UINT8 uLang, UINT32 iStrID, const GUI_BITMAP* IconID, UINT32 barId, GUI_COLOR clrFont, GUI_COLOR clrBack, GUI_COLOR clrSelect)
{
    ...
    if(AHC_TRUE == AHC_Charger_GetStatus())
    {
	   ...
	   //if (IconID != NULL && IconID != &bmIcon_Dummy)
	    {
	    	 printc("DrawSubItem-ICONID-iItem=%d,iMenuId=%d\n",iItem,iMenuId);
		if(iMenuId==MENUID_SUB_MENU_SETTING)
		{
		    	 switch(iItem)
			{
				case 0:
		...
		else if(iMenuId==MENUID_SUB_MENU_SETUP)
		{
		    	 switch(iItem)
			{
				case 0:
		...
		if(iMenuId==MENUID_SUB_MENU_SETUP)
		{
			if(iItem == 4)
			{
					if(display_flag2 == 0)
					{
						OSD_Draw_Icon(bmm1, rc, uwDispID);
					}
					if(display_flag2 == 1)
					{
						OSD_Draw_Icon(bmm2, rc, uwDispID);
					}
					if(display_flag2 == 2)
					{
						OSD_Draw_Icon(bmm3, rc, uwDispID);
					}
			}
			if(iItem == 5)
			{
					if(display_flag3 == 0)
					{
						OSD_Draw_Icon(bmnight1_on, rc, uwDispID);
					}
					if(display_flag3 == 1)
					{
						OSD_Draw_Icon(bmnight2_off, rc, uwDispID);
					}
					if(display_flag3 == 2)
					{
						OSD_Draw_Icon(bmnight3_parking, rc, uwDispID);
					}
			}
			if(iItem != 4 && iItem != 5)
...
/*void DrawSubItemResolution(UINT16 uwDispID, int iItem, int iTotalItems, UINT8 uLang, UINT32 iStrID, const GUI_BITMAP* IconID, UINT32 barId, GUI_COLOR clrFont, GUI_COLOR clrBack, GUI_COLOR clrSelect)
{
		...
		const GUI_BITMAP* 	SubMenuLcdPushIcon[]={&bmIcon_hd2_button,
													&bmIcon_hd_button,
													};
		#else
		...
		#if (MENU_MOVIE_SIZE_1080P_EN||MENU_MOVIE_SIZE_SHD_30P_EN)
		rc.uiLeft 	     =65+iItem%2*(IconID->XSize+120);
		#else
		rc.uiLeft 	     =185+iItem%2*(IconID->XSize+120);
		#endif
...
void DrawSubItemEx(UINT16 uwDispID, PSMENUSTRUCT pMenu)
{
    ...
    uLang  = MenuDraw_GetDefaultLanguage(pMenu);
	 printc("DrawSubItemEx---pMenu->iCurrentPos=%d,pMenu->iNumOfItems=%d\n",pMenu->iCurrentPos, pMenu->iNumOfItems);

    iBegin = ALIGN_DOWN( pMenu->iCurrentPos, uPageItemNum );
    iEnd   = MIN( iBegin + uPageItemNum, pMenu->iNumOfItems );
	printc("DrawSubItemEx---iBegin=%d,iEnd=%d\n",iBegin,iEnd);
    ...
    for( i=iBegin; i<iEnd; i++ )
    {
	    ...
	    if(pMenu->iMenuId==MENUID_SUB_MENU_SETTINGVOICE)
	    {
	...
	printc("i==%dbarType=%dbarState=%d\n",i,barType,barState);

	printc("DrawSubItemEx ---pMenu->iMenuId%d\n",pMenu->iMenuId);
	switch(pMenu->iMenuId)
	{
		case MENUID_SUB_MENU_LCD:
			 DrawSubItemLcd(uwDispID, idx, pMenu->iNumOfItems,
		...
		case MENUID_SUB_MENU_LOGO:
			 DrawSubItemLogo(uwDispID, idx, pMenu->iNumOfItems,
	                    uLang, pMenu->pItemsList[i]->uiStringId,
	                    pMenu->pItemsList[i]->bmpIcon,
	                    MAKE_BAR_ID(barType, barState),
	                    TextClr, bkColor, 0x0);
		...
		case MENUID_SUB_MENU_GSENSOR:
			 DrawSubItemGsensor(uwDispID, idx, pMenu->iNumOfItems,
	                    uLang, pMenu->pItemsList[i]->uiStringId,
	                    pMenu->pItemsList[i]->bmpIcon,
	                    MAKE_BAR_ID(barType, barState),
	                    TextClr, bkColor, 0x0);
		...
		case MENUID_SUB_MENU_MIC:
			DrawSubItemYesNo1(uwDispID, idx, pMenu->iNumOfItems,
	                    uLang, pMenu->pItemsList[i]->uiStringId,
	                    pMenu->pItemsList[i]->bmpIcon,
	                    MAKE_BAR_ID(barType, barState),
	                    TextClr, bkColor, 0x0);
		...
		case MENUID_SUB_MENU_GPS:
			DrawSubItemYesNo3(uwDispID, idx, pMenu->iNumOfItems,
	                    uLang, pMenu->pItemsList[i]->uiStringId,
	                    pMenu->pItemsList[i]->bmpIcon,
	                    MAKE_BAR_ID(barType, barState),
	                    TextClr, bkColor, 0x0);
		...
		case MENUID_SUB_MENU_SETTINGTOUCH:	
			 DrawSubItemYesNo(uwDispID, idx, pMenu->iNumOfItems,
	                    uLang, pMenu->pItemsList[i]->uiStringId,
	                    pMenu->pItemsList[i]->bmpIcon,
	                    MAKE_BAR_ID(barType, barState),
	                    TextClr, bkColor, 0x0);
		...
		case MENUID_SUB_MENU_SETTINGPARKING:
			DrawSubItemYesNo4(uwDispID, idx, pMenu->iNumOfItems,
	                    uLang, pMenu->pItemsList[i]->uiStringId,
	                    pMenu->pItemsList[i]->bmpIcon,
	                    MAKE_BAR_ID(barType, barState),
	                    TextClr, bkColor, 0x0);
		...
		case MENUID_SUB_MENU_SETTINGBATTERYH:
			DrawSubItemYesNo5(uwDispID, idx, pMenu->iNumOfItems,
	                    uLang, pMenu->pItemsList[i]->uiStringId,
	                    pMenu->pItemsList[i]->bmpIcon,
	                    MAKE_BAR_ID(barType, barState),
	                    TextClr, bkColor, 0x0);
		...
		case MENUID_SUB_MENU_SETTINGVOLT2:
			 DrawSubItemsettingvolt(uwDispID, idx, pMenu->iNumOfItems,
	                    uLang, pMenu->pItemsList[i]->uiStringId,
	                    pMenu->pItemsList[i]->bmpIcon,
	                    MAKE_BAR_ID(barType, barState),
	                    TextClr, bkColor, 0x0);
		...
		case MENUID_SUB_MENU_MOVIE_MODE:
			DrawSubItemResolution(uwDispID, idx, pMenu->iNumOfItems,
	                    uLang, pMenu->pItemsList[i]->uiStringId,
	                    pMenu->pItemsList[i]->bmpIcon,
	                    MAKE_BAR_ID(barType, barState),
	                    TextClr, bkColor, 0x0);
		...
		case MENUID_SUB_MENU_SETTINGFORMATE:
			DrawSubItemFormat(uwDispID, idx, pMenu->iNumOfItems,
	                    uLang, pMenu->pItemsList[i]->uiStringId,
	                    pMenu->pItemsList[i]->bmpIcon,
	                    MAKE_BAR_ID(barType, barState),
	                    TextClr, bkColor, 0x0);
		...
		case MENUID_SUB_MENU_SETTINGVOICE:
			 DrawSubItemsettingvoice(uwDispID, idx, pMenu->iNumOfItems,
	                    uLang, pMenu->pItemsList[i]->uiStringId,
	                    pMenu->pItemsList[i]->bmpIcon,
	                    MAKE_BAR_ID(barType, barState),
	                    TextClr, bkColor, 0x0);
		...
		case MENUID_SUB_MENU_SETTINGTIME:
			  printc("MENUID_SUB_MENU_SETTINGTIME %d\n",i );
			 DrawSubItem_ClockSetting1(uwDispID, i ,0);
		break;
		case MENUID_SUB_MENU_SETTINGSYSTEM:
			DrawSubItemResolutionEX2(uwDispID, idx, pMenu->iNumOfItems,
	                    uLang, pMenu->pItemsList[i]->uiStringId,
	                    pMenu->pItemsList[i]->bmpIcon,
	                    MAKE_BAR_ID(barType, barState),
	                    TextClr, bkColor, 0x0);
		...
		case MENUID_SUB_MENU_PLAYBACK:
			 DrawSubItemPLAYBACK(uwDispID, idx, pMenu->iNumOfItems,
	                    uLang, pMenu->pItemsList[i]->uiStringId,
	                    pMenu->pItemsList[i]->bmpIcon,
	                    MAKE_BAR_ID(barType, barState),
	                    TextClr, bkColor, 0x0);
...
void MenuDrawSubTitle(UINT16 uwDispID, UINT32 iStrID)
{
    ...
    #if (SUPPORT_TOUCH_PANEL)
    RECT RECTExit = POS_MENU_EXIT;
    RECT RECTSetupBg= {0,80,480,320};

    OSD_Draw_Icon(bmIcon_home_button3_normal, RECTExit, uwDispID);
...
AHC_BOOL select_time_flag = 0; 
void MenuDrawSubPageR1(UINT8 uwDispID, PSMENUSTRUCT pMenu)
{
      RECT RECTSetupBg= {0,80,480,320};
      RECT RECTSetupHome= {335,0,145,70};
      RECT RECTSetupLcd= {0,0,300,70};
      RECT RECTSetupReset= {0,0,480,320};
	  RECT RECTSettingvolt= {0,0,480,84};
	  RECT RECTSettingvoltTitle= {320,120,115,70};
	  RECT RECTSettingTouchSound= {20,220,190,70};
	  RECT RECTSettingTVolTile={20,120,100,70};
	  RECT RECTSettingFormat1= {30,115,310,40};
	  RECT RECTSetupFormat2= {360,115,90,40};
	  RECT RECTSetupHome1= {118,100,124,240};
	  RECT RECTSetupHome2= {246,100,124,240};
	  RECT RECTSetupHome3= {10,215,150,100};
	  RECT RECTSetupHome4= {10,193,150,122};
	  RECT RECTSetupHome5= {0,202,160,118};
	   RECT RECTSetupHome6= {0,135,96,150};
	    RECT RECTSetupHome7= {290,30,145,90};
	RECT RECTExit = POS_MENU_EXIT;
    if (GetSubMenuType()==SUBMENU_SUBPAGE)
    {
    	 ...
    	 {
    	 	case MENUID_SUB_MENU_LCD:
			OSD_Draw_Icon(bmIcon_setup_bg_bar,RECTSetupBg,uwDispID);
			OSD_Draw_Icon(bmIcon_home_button_normal,RECTSetupHome,uwDispID);
			OSD_Draw_Icon(bmIcon_lcd_title,RECTSetupLcd,uwDispID);
			break;
		case MENUID_SUB_MENU_LOGO:
			OSD_Draw_Icon(bmIcon_setup_bg_bar,RECTSetupBg,uwDispID);
			OSD_Draw_Icon(bmIcon_home_button_normal,RECTSetupHome,uwDispID);
			OSD_Draw_Icon(bmIcon_logo_title,RECTSetupLcd,uwDispID);
			OSD_Draw_Icon(bmIcon_1_bt_normal,RECTSetupHome3,uwDispID);
			break;
		case MENUID_SUB_MENU_GSENSOR:
			OSD_Draw_Icon(bmIcon_setup_bg_bar,RECTSetupBg,uwDispID);
			OSD_Draw_Icon(bmIcon_home_button_normal,RECTSetupHome,uwDispID);
			OSD_Draw_Icon(bmIcon_sensor_title,RECTSetupLcd,uwDispID);
			break;
		case MENUID_SUB_MENU_MIC:
			OSD_Draw_Icon(bmIcon_setup_bg_bar,RECTSetupBg,uwDispID);
			OSD_Draw_Icon(bmIcon_home_button_normal,RECTSetupHome,uwDispID);
			OSD_Draw_Icon(bmIcon_mic_title,RECTSetupLcd,uwDispID);
			break;
		case MENUID_SUB_MENU_RESET:
			OSD_Draw_Icon(bmIcon_setup_bg_bar,RECTSetupBg,uwDispID);
			OSD_Draw_Icon(bmIcon_home_button_normal,RECTSetupHome,uwDispID);
			//OSD_Draw_Icon(bmIcon_reset_title,RECTSetupReset,uwDispID);
			OSD_Draw_Icon(bmsm1,RECTSetupHome4,uwDispID);
			break;
		case MENUID_SUB_MENU_GPS:
			OSD_Draw_Icon(bmIcon_setup_bg_bar,RECTSetupBg,uwDispID);
			OSD_Draw_Icon(bmIcon_home_button_normal,RECTSetupHome,uwDispID);
			//OSD_Draw_Icon(bmIcon_gps_title,RECTSetupReset,uwDispID);
			OSD_Draw_Icon(bmnight1_son,RECTSetupHome4,uwDispID);
				break;
		case MENUID_SUB_MENU_SETTINGEVENT:
			//OSD_Draw_Icon(bmIcon_setup_bg_bar,RECTSetupBg,uwDispID);
			//OSD_Draw_Icon(bmIcon_home_button_normal,RECTSetupReset,uwDispID);
			OSD_Draw_Icon(bmevent_bg,RECTSetupReset,uwDispID);
				break;
		case MENUID_SUB_MENU_SETTINGPARKING:
			OSD_Draw_Icon(bmIcon_setup_bg_bar,RECTSetupBg,uwDispID);
			OSD_Draw_Icon(bmIcon_home_button_normal,RECTSetupHome,uwDispID);
			OSD_Draw_Icon(bmparkingselect1,RECTSetupHome5,uwDispID);
				break;
		case MENUID_SUB_MENU_SETTINGFORMATE:
			OSD_Draw_Icon(bmIcon_setup_bg_bar,RECTSetupBg,uwDispID);
			OSD_Draw_Icon(bmIcon_home_button_normal,RECTSetupHome,uwDispID);
			OSD_Draw_Icon(bmIcon_format03_title,   RECTSetupLcd,uwDispID);
			OSD_Draw_Icon(bmIcon_Format_title1,   RECTSettingFormat1,uwDispID);
			OSD_Draw_Icon(bmIcon_Format_title2,   RECTSetupFormat2,uwDispID);
			break;
		case MENUID_SUB_MENU_SETTINGFORMATE1:
			OSD_Draw_Icon(bmformat_bg,RECTSetupReset,uwDispID);
				break;
		case MENUID_SUB_MENU_SETTINGTOUCH:
			//OSD_Draw_Icon(bmIcon_setup_bg_bar,RECTSetupBg,uwDispID);
			//OSD_Draw_Icon(bmIcon_home_button_normal,RECTSetupReset,uwDispID);
			OSD_Draw_Icon(bmtouch_bg,RECTSetupReset,uwDispID);
			break;
		case MENUID_SUB_MENU_SETTINGTIME:
			//OSD_Draw_Icon(bmIcon_setup_bg_bar,RECTSetupBg,uwDispID);
			//OSD_Draw_Icon(bmIcon_home_button_normal,RECTSetupReset,uwDispID);
			OSD_Draw_Icon(bmtime_bg,RECTSetupReset,uwDispID);
				break;
		case MENUID_SUB_MENU_SETTINGSYSTEM:
			//OSD_Draw_Icon(bmIcon_setup_bg_bar,RECTSetupBg,uwDispID);
			//OSD_Draw_Icon(bmIcon_home_button_normal,RECTSetupReset,uwDispID);
			OSD_Draw_Icon(bmIcon_home_button4_normal, RECTExit, uwDispID);
			OSD_Draw_Icon(bmvoltsetting,   RECTSetupHome1,uwDispID);
			#if 0
			if(select_time_flag == 0)
				OSD_Draw_Icon(bmbatterysetting,   RECTSetupHome2,uwDispID);
			if(select_time_flag == 1)
				OSD_Draw_Icon(bmsfivehour,   RECTSetupHome2,uwDispID);
			if(select_time_flag == 2)
				OSD_Draw_Icon(bmstenhour,   RECTSetupHome2,uwDispID);
			if(select_time_flag == 3)
				OSD_Draw_Icon(bmstwentyfour,   RECTSetupHome2,uwDispID);
			if(select_time_flag == 4)
				OSD_Draw_Icon(bmsfortyeight,   RECTSetupHome2,uwDispID);
			#endif
			#if 0
			OSD_Draw_Icon(bmsystem_bg,RECTSetupReset,uwDispID);
			AHC_OSDSetFont(uwDispID, &GUI_Font32B_1);
	    		AHC_OSDSetColor(uwDispID, OSD_COLOR_RED);
			AHC_OSDSetBkColor(uwDispID, 0x00050505);
	    		AHC_OSDDrawText(uwDispID, (UINT8*)"VER:SCI",  100, 150, strlen("VER:SCI"));	
			AHC_OSDDrawDec(uwDispID, 20170320,     230,    150, 8);
			#endif
				break;
		case MENUID_SUB_MENU_SETTINGBATTERYH:
			OSD_Draw_Icon(bmIcon_home_button4_normal, RECTExit, uwDispID);
			OSD_Draw_Icon(bmbatterysetting2,   RECTSetupHome6,uwDispID);
			break;
		case MENUID_SUB_MENU_MOVIE_MODE:
			OSD_Draw_Icon(bmIcon_setup_bg_bar,RECTSetupBg,uwDispID);
			OSD_Draw_Icon(bmIcon_home_button_normal,RECTSetupHome,uwDispID);
			OSD_Draw_Icon(bmIcon_hd_bg_title,   RECTSetupLcd,uwDispID);
			OSD_Draw_Icon(bmIcon_hd2_button,   RECTSetupHome1,uwDispID);
			if(attach_flag2)
				OSD_Draw_Icon(bmIcon_hd_button,   RECTSetupHome2,uwDispID);
			else
			    OSD_Draw_Icon(bmIcon_hd__bt_nor,   RECTSetupHome2,uwDispID);

			break;
		case MENUID_SUB_MENU_PLAYBACK:
			OSD_Draw_Icon(bmIcon_setup_bg_bar,   RECTSetupBg,uwDispID);
		   	OSD_Draw_Icon(bmIcon_home_button_normal,   RECTSetupHome,uwDispID);
		   	OSD_Draw_Icon(bmIcon_play_title,   RECTSetupLcd,uwDispID);
			break;
		case MENUID_SUB_MENU_SETTINGVOICE:
			//OSD_Draw_Icon(bmtouch_sound,   RECTSettingTouchSound,uwDispID);
			OSD_Draw_Icon(bmIcon_vol_title02,   RECTSettingTVolTile,uwDispID);
			OSD_Draw_Icon(bmIcon_setup_bg_bar,   RECTSetupBg,uwDispID);
		   	OSD_Draw_Icon(bmIcon_home_button_normal,   RECTSetupHome,uwDispID);
		   	OSD_Draw_Icon(bmIcon_voice_title,   RECTSetupLcd,uwDispID);

			break;
		case MENUID_SUB_MENU_SETTINGVOLT:
		case MENUID_SUB_MENU_SETTINGVOLT1:
		case MENUID_SUB_MENU_SETTINGVOLT2:	
			OSD_Draw_Icon(bmIcon_volt_volt01,   RECTSetupLcd,uwDispID);
			OSD_Draw_Icon(bmIcon_setup_bg_bar,   RECTSetupBg,uwDispID);
		   	OSD_Draw_Icon(bmIcon_home_button_normal,   RECTSetupHome,uwDispID);
		   	//OSD_Draw_Icon(bmvolt_title02,   RECTSettingvoltTitle,uwDispID);
			break;
		case MENUID_SUB_MENU_SETUP:
			OSD_Draw_Icon(bmIcon_home_button3_normal, RECTExit, uwDispID);
			break;
		case MENUID_SUB_MENU_SETTING:
    			OSD_Draw_Icon(bmIcon_home_button4_normal, RECTExit, uwDispID);
			break;
		default:
			if( pMenu->uiStringId != -1 )
...
void MenuDrawChangeSubItem(PSMENUSTRUCT pMenu, UINT32 uiCurrItem, UINT32 uiPrevItem, UINT32 uiPreSelected)//leo20180630
{
    UINT8     bID0 = 0;
    UINT8     bID1 = 0;
	...
	const GUI_BITMAP* 	SubMenuSettingPushIcon[]={&bmvolt_bt_push,
											&bmsound_button_push,
											&bmformat_button_push,
											&bmtime_button_push,
											&bmevent_bt_push,
											&bmtouch_button_push,
											&bmsystem_button_push,
											&bmParkingmode,
											};	
		
			const GUI_BITMAP* 	SubMenuSetupPushIcon[]={&bmIcon_lcd_button_push,
											&bmIcon_start_screen_button_push,
											&bmIcon_sensor_button_push,
											&bmIcon_mic_button_push,
											&bmIcon_reset_button_push,
											&bmIcon_gps_button_push,};	
			
	 uLang = MenuDraw_GetDefaultLanguage(pMenu);
	 printc("MenuDrawChangeSubItem---uiPrevItem=%d,uiCurrItem=%d\n",uiPrevItem,uiCurrItem);
        OSDDraw_EnterMenuDrawing(&bID0, &bID1);
...
//Draw uiCurrItem 
	idx=uiCurrItem;
		if(pMenu->iMenuId==MENUID_SUB_MENU_SETTING	)
				{

    	DrawSubItem(bID0, idx, pMenu->iNumOfItems,
                        uLang, pMenu->pItemsList[uiCurrItem]->uiStringId,
                        SubMenuSettingPushIcon[uiCurrItem],
                        MAKE_BAR_ID(barType, barState),
                        clrText, clrFill, clrSelect);
		}else{
						DrawSubItem(bID0, idx, pMenu->iNumOfItems,
									uLang, pMenu->pItemsList[uiCurrItem]->uiStringId,
									SubMenuSetupPushIcon[uiCurrItem],
									MAKE_BAR_ID(barType, barState),
									clrText, clrFill, clrSelect);
...
void MenuDrawChangeSubItemfoure(PSMENUSTRUCT pMenu, UINT32 uiCurrItem, UINT32 uiPrevItem, UINT32 uiPreSelected)
{
    UINT8     bID0 = 0;
    UINT8     bID1 = 0;
    UINT8     SubPageItem = SUB_MENU_PAGE_ITEM;
    ...
    uLang = MenuDraw_GetDefaultLanguage(pMenu);
    {
	 ...
	 printc("MenuDrawChangeSubItemThird ---pMenu->uiCurrItem=%d\n",uiCurrItem);
		...
		{
			case MENUID_SUB_MENU_MOVIE_MODE:
				printc("~~FFFF~~HD~~~MENUID_SUB_MENU_MOVIE_MODE~~~~~~\r\n");
		...
		switch(pMenu->iMenuId)
		{
			case MENUID_SUB_MENU_MOVIE_MODE:
				...
				printc("~2222~FFF~~HD~~~MENUID_SUB_MENU_MOVIE_MODE~~~~~~\r\n");
				DrawSubItemResolution(bID0, idx, pMenu->iNumOfItems,
		                    uLang, pMenu->pItemsList[uiCurrItem]->uiStringId,
		                    pMenu->pItemsList[uiCurrItem]->bmpIcon,
		                    MAKE_BAR_ID(barType, barState),
...
void MenuDrawSubPage_ConfirmPage(PSMENUSTRUCT pMenu)
{
    ...
    OSDDraw_ClearOvlDrawBuffer(bID0);
    MenuDrawSubBackCurtain(bID0, OSD_COLOR_BLACK);//leo20180630
