...
#ifndef	_OEM_TOUCH_
extern AHC_BOOL                    bMuteRecord ;//leo20180630
...
void OSDDraw_MainTouchButton(UINT16 ubID, UINT8 uiMode)
{
    ...
    RECT  RectMode 	= RECT_TOUCH_BUTTON_FRONT_BACK_CAM;	
    RECT  FrontCamMode 	= RECT_TOUCH_BUTTON_FRONT_CAM;	
    RECT  BackCamMode 	= RECT_TOUCH_BUTTON_BACK_CAM;	
    RECT  FrontBackCamMode 	= RECT_TOUCH_BUTTON_FRONT_BACK_CAM;	
    RECT  MuteMode 	= RECT_TOUCH_BUTTON_MUTE;	
	RECT RECT_TOUCH_BUTTON_REC= RECT_TOUCH_BUTTON_OK1;
    if (AHC_FALSE == AIHC_GetOSDInitStatus()) { AHC_PRINT_RET_ERROR(gbAhcDbgBrk,0); return; }
    ...
    AHC_OSDSetColor(ubID, 0xAA000000); 
	 ...
	 /*#if (MENU_MOVIE_SIZE_1080P_EN||MENU_MOVIE_SIZE_SHD_30P_EN)
	    if(MenuSettingConfig()->uiMOVSize)
	    	{
		    AHC_OSDDrawBitmap(ubID, &bmIcon_front_cam_push, 	FrontCamMode.uiLeft, 	 FrontCamMode.uiTop);
		    AHC_OSDDrawBitmap(ubID, &bmIcon_back_cam_normal, 	BackCamMode.uiLeft, 	 BackCamMode.uiTop);
	    	}
		else
		{
			    AHC_OSDDrawBitmap(ubID, &bmIcon_front_cam_normal, 	FrontCamMode.uiLeft, 	 FrontCamMode.uiTop);
		    	AHC_OSDDrawBitmap(ubID, &bmIcon_back_cam_push, 	BackCamMode.uiLeft, 	 BackCamMode.uiTop);
		}
	#endif
	//AHC_OSDDrawBitmap(ubID, &bmIcon_front_back_cam_normal, 						FrontBackCamMode.uiLeft, 	 FrontBackCamMode.uiTop);
	   	 if(bMuteRecord)
	   	AHC_OSDDrawBitmap(ubID, &bmIcon_mute, MuteMode.uiLeft, 	 MuteMode.uiTop);
	   else
	   	AHC_OSDDrawBitmap(ubID, &bmIcon_no_mute, MuteMode.uiLeft, 	 MuteMode.uiTop);

    // AHC_OSDDrawBitmap(ubID, &bmIcon_Button_Zoom_In, mRECT.uiLeft, mRECT.uiTop);
#if (MENU_MOVIE_SIZE_1080P_EN||MENU_MOVIE_SIZE_SHD_30P_EN)
  if(VideoFunc_RecordStatus())
    		AHC_OSDDrawBitmap(ubID, &bmIcon_stop, 						RECT_TOUCH_BUTTON_REC.uiLeft, 	 RECT_TOUCH_BUTTON_REC.uiTop);
   else 
   		AHC_OSDDrawBitmap(ubID, &bmIcon_rec_button_push, 						RECT_TOUCH_BUTTON_REC.uiLeft, 	 RECT_TOUCH_BUTTON_REC.uiTop);   		
  #endif
  */
    		//AHC_OSDDrawBitmap(ubID, &bmIcon_take_video, 						RectOK.uiLeft, 	 RectOK.uiTop);   		
    //AHC_OSDDrawBitmap(ubID, &bmmenu_button, 						RectMenu.uiLeft, RectMenu.uiTop);
    //OSD_ShowStringPool3(ubID, IDS_DS_MENU, RectMenu, OSD_COLOR_WHITE, OSD_COLOR_TRANSPARENT, GUI_TA_HCENTER|GUI_TA_VCENTER);
    //AHC_OSDDrawBitmap(ubID, &bmIcon_pip_button_normal, 		RectMode.uiLeft, RectMode.uiTop);	//leo20180630	

    if(uiGetParkingModeEnable())
    		AHC_OSDDrawBitmap(ubID, &bmparking_rec, 0,	95);
    if(VideoFunc_LockFileEnabled())
		AHC_OSDDrawBitmap(ubID, &bmgsensor_rec, 0,20);

    END_LAYER(ubID);
#endif
}
#endif
void OSDDraw_MainTouchplayback(UINT16 ubID, UINT8 uiMode)
{
#if (SUPPORT_TOUCH_PANEL)
    ...
    BEGIN_LAYER(ubID);
    AHC_OSDSetColor(ubID, 0xAA000000);
    AHC_OSDDrawBitmap(ubID, &bmdel_button_normal, 						DELBOTTON.uiLeft, 	 DELBOTTON.uiTop);
    AHC_OSDDrawBitmap(ubID, &bmback1_button_normal, 						BACK1BOTTON.uiLeft, 	 BACK1BOTTON.uiTop);
    AHC_OSDDrawBitmap(ubID, &bmplay_button2_normal, 						PLAYBOTTON.uiLeft, 	 PLAYBOTTON.uiTop);
    AHC_OSDDrawBitmap(ubID, &bmnext1_button_normal, 						NEXT1BOTTON.uiLeft, 	 NEXT1BOTTON.uiTop);

    // AHC_OSDDrawBitmap(ubID, &bmIcon_Button_Zoom_In, mRECT.uiLeft, mRECT.uiTop);
    AHC_OSDDrawBitmap(ubID, &bmhome_button2_normal, 						HOMEBOTTON.uiLeft, 	 HOMEBOTTON.uiTop);
    //AHC_OSDDrawBitmap(ubID, &bmmenu_button, 						RectMenu.uiLeft, RectMenu.uiTop);
   // OSD_ShowStringPool3(ubID, IDS_DS_MENU, RectMenu, OSD_COLOR_WHITE, OSD_COLOR_TRANSPARENT, GUI_TA_HCENTER|GUI_TA_VCENTER);
    //AHC_OSDDrawBitmap(ubID, &bmIcon_pip_button_normal, 		RectMode.uiLeft, RectMode.uiTop);		
	 END_LAYER(ubID);
#endif
}
...
void OSDDraw_CurrentRTCTime(UINT16 ubID, UINT32 x, UINT32 y, AHC_BOOL bLarge, UINT8 ubMag, AHC_BOOL bFlag, GUI_COLOR bkColor, INT32 Para1)
{
    ...
    {	
	 ...
	 if(1){
				...
				//   printc("OSDDraw_CurrentRTCTime:ADC voltage = %d\r\n", saradc_data);
				       if(saradc_data>=2000)
					   	saradc_data=2000;
						TEMP_data=95-saradc_data*47/1000;
				 	//else if(saradc_data>=1400)
				 	//	TEMP_data=132-saradc_data*51/1000;
					 
				l= sprintf(szvolt, "T:%d c  ",TEMP_data);
				 AHC_OSDDrawText(ubID, (UINT8*)szvolt, 330 ,300, l);

				 pf_General_EnGet(COMMON_KEY_B_VOL, &level);
				if(level<820)
					{
						level1=(level*100/791)%10;
						level=level*10/791;
						
					}
					else if(level<1300){
								
								level1=(level*100/758)%10;
							level=level*10/758;

							}
			    	
				else {
						level1=(level*100/694)%10;
						level=level*10/694;
					}
				l= sprintf(szvolt, "%d.%dv  ", level,level1);
				
		            AHC_OSDDrawText(ubID, (UINT8*)szvolt, 180 ,300, l);
		}
    }
    else
   	{
