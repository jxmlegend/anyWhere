...
//MMP_UBYTE  Temp1[4];
MMP_SHORT tpRefCenterX = 1920, tpRefCenterY = 1760, tpStepX = 8, tpStepY = 11, tptemp = 0;

/*===========================================================================
 * Global varible
 *===========================================================================*/
//leo20180630
static MMP_I2CM_ATTR  m_TouchPanelI2c = {MMP_I2CM2, TOUCH_PANEL_I2C_SLAVE_ADDRESS, 8, 8, 0x20, MMP_FALSE, MMP_FALSE, MMP_FALSE, MMP_FALSE, 0x0, 0x0, 0x0, 80000 /*80KHZ*/, MMP_TRUE, NULL, NULL, MMP_TRUE, MMP_TRUE, MMP_FALSE};
...
void TouchPanel_TransformPosition(MMP_USHORT* pX, MMP_USHORT* pY, MMP_UBYTE dir) // Set dir = 0 as lcd normal, 1: reverse.
{
	...
	{	
		...
		printc("111111111111111111111111111111111111111111111111111\r\n");	
			printc("tpRefCenterX=%d***********tpRefCenterY=%d***********tpStepX=%d***********tpStepY=%d******\r\n",tpRefCenterX,tpRefCenterY,tpStepX,tpStepY);	

			*pX =  60+100*(*pX-tpRefCenterX)/tpStepX;

			*pY = 60+100*(*pY-tpRefCenterY)/tpStepY;
			//printc("tpRefCenterX=%d***********tpRefCenterY=%d***********tpStepX=%d***********tpStepY=%d******\r\n",tpRefCenterX,tpRefCenterY,tpStepX,tpStepY);	
...
int TouchPanel_ReadPosition(struct _3RD_PARTY_TOUCHPANEL *pthis, MMP_USHORT* pX, MMP_USHORT* pY, MMP_UBYTE dir)
{
    ...
    Temp[0] = usReadData[0];
    Temp[1] = usReadData[1];
   // printc("temp111 %x %x  ", Temp[0], Temp[1]);
    *pX = Temp[0]*16+Temp[1]/16;
	...
	Temp[0] = usReadData[0];
    Temp[1] = usReadData[1];
    printc("temp222 %x %x  ", Temp[0], Temp[1]);
    *pY = Temp[0]*16+Temp[1]/16;
    ...
    MMPF_I2cm_WriteReg(&m_TouchPanelI2c, NULL, usWriteData);
    MMPF_I2cm_ReadBurstData(&m_TouchPanelI2c, NULL, usReadData, 0x2);
   // printc("temp333 %x %x  ", Temp[0], Temp[1]);
    //	printc("leo pX %d  pY %d\r\n", *pX,  *pY);
	//printc("leo dir=%d\n",dir);
	TouchPanel_TransformPosition(pX,pY,dir);
	//printc("TouchPanel_TransformPosition -->pX %d  pY %d\r\n",*pX,  *pY);
    if( usReadData[0] == 0 )
    {
        //printc("pX 0  pY 0\r\n");
	    *pX = 0;
...
MMP_ERR TouchPanel_IOControl(struct _3RD_PARTY_TOUCHPANEL *pthis, TOUCHPANEL_ATTR *ptouchpanel_attr, MMP_UBYTE cmd, MMP_USHORT* arg)
{
    ...
    *pX = Temp[0]*16+Temp[1]/16;
	...
	Temp[2] = usReadData[0];
    Temp[3] = usReadData[1];
    //printc("temp %x %x  ", Temp[2], Temp[3]);
    *pY = Temp[2]*16+Temp[3]/16;


    	//printc("leo pX %d  pY %d\r\n", *pX,  *pY);
	//printc("leo dir=%d\n",dir);
	//printc("**********************TouchPanel_TransformPosition*****************\r\n");
	*arg=Temp[0];
	*(arg+1)=Temp[1];
	*(arg+2)=Temp[2];
	*(arg+3)=Temp[3];
		...
		tpStepX=*(arg+2);
		tpStepY=*(arg+3);
		//tpStepX=(tpStepX+50)/100;
	//	tpStepY=(tpStepY+50)/100;
		tptemp = 1;
		//printc("tpRefCenterX=%d***********tpRefCenterY=%d***********tpStepX=%d***********tpStepY=%d******\r\n",tpRefCenterX,tpRefCenterY,tpStepX,tpStepY);	
